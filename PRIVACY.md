...
- OneHome is supported only through sanitized snapshots. Token-free property paths may be published, but access tokens, token-bearing URLs, contact identifiers, and private email addresses are rejected by validation and must never be committed. The dashboard may combine a pasted current OneHome URL with those property paths in memory and share that connection with other open Property Radar tabs through a same-origin browser channel; it does not use browser storage. A private fragment may initialize that connection without sending the token to GitHub Pages, and the fragment is immediately removed from the address bar.
