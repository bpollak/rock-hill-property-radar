...
- For every candidate, explicitly reconcile current marketing status against the dated sale and transfer chronology. A prior sale may be followed by a legitimate new listing, but the output must show the intervening relist event. A property page, contact form, or cached search result is not proof that a home is currently for sale.
- Confirm status from the direct MLS-fed source-status field and, when publicly available, a second current syndicated or public IDX source. If credible current sources conflict, preserve the conflict and use the stricter status: pending or contingent over active, and confirmed sold or off-market over a stale active page.
- Never infer active status merely because photos, an estimated payment, an agent-contact form, or a historical listing page remains online. Record the source listing status, its last-update date when exposed, the most recent sale date, and any later list or relist date.
...
- Include a current Rock Hill studio/one-bedroom rental benchmark. Maintain current, deduplicated private-bedroom asking-rent evidence for every regional market zone represented by active shared-property candidates. Capture ZIP, bathroom access, furnishing, utilities, parking, property type, lease length, status, update date, source URL, and source provider. Exclude whole-unit rentals and shared-bedroom occupancy. Treat asking rents as evidence, not signed leases, and keep sparse or single-source zones low confidence.
